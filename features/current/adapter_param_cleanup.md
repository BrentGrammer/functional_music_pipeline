...
1. Cleanup batch-two adapters only
   - Remove duplicate schema-style validation from weierstrass_phrase_transform and terraced_drift_phrase_transform.
   - Keep only minimal param extraction needed to call the raw functions.
   - Do not touch batch three yet.
   - Run the batch-two tests.

2. Fix raw terraced_drift bool handling
   - Update apply_terraced_drift_transform so max_step_change_pct=True/False is rejected.
   - Add one focused test.
   - Run tests/test_geological_modulation.py.
3. Then proceed to batch three using the cleaner rule
   - New adapters should rely on params_spec for public validation.
   - Raw transforms keep their own direct-call protections.
   - Run the batch-three tests.
