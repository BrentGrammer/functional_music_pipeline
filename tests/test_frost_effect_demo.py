...
def test_frost_single_seed_demo_loads_and_sequences_original_then_frost():
    ...
    assert len(flatten_voice_tones(score.voices[0])) == 1
    assert len(flatten_voice_tones(score.voices[1])) == 2
    assert len(flatten_voice_tones(score.voices[2])) == 2
    assert len(flatten_voice_tones(score.voices[3])) == 2
    assert len(flatten_voice_tones(score.voices[4])) == 2
    assert len(flatten_voice_tones(score.voices[5])) == 2
    assert len(flatten_voice_tones(score.voices[6])) == 2
    assert len(flatten_voice_tones(score.voices[7])) == 2
    assert len(flatten_voice_tones(score.voices[8])) == 2
    assert len(flatten_voice_tones(score.voices[9])) == 2
    assert len(flatten_voice_tones(score.voices[10])) == 2
    assert len(flatten_voice_tones(score.voices[11])) == 2
    assert len(flatten_voice_tones(score.voices[12])) == 2
    assert len(flatten_voice_tones(score.voices[13])) == 2
    assert len(flatten_voice_tones(score.voices[14])) == 2
    assert len(flatten_voice_tones(score.voices[15])) == 2
    ...
    first_event_frequencies = {flatten_voice_tones(voice)[1].frequency for voice in first_event_voices}

    assert flatten_voice_tones(score.voices[1])[0].frequency == 0
    assert flatten_voice_tones(score.voices[1])[1].duration == pytest.approx(
        flatten_voice_tones(score.voices[0])[0].duration
    )
