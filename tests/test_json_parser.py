...
def render_phrase_from_config(
    ...
    if reference_tones is not None:
        motifs_section["__reference__"] = [
            f"{tone.frequency}:{tone.duration}" for tone in reference_tones
        ]
    ...
    composition_doc: object = {
        ...
        "score": {"motifs": motifs_section, "voices": composition_voices},
    }
...
def test_tempo_curve_descriptor_accepts_preset_or_float_controls(transform_name):
    ...
    assert fields["strength"].schema[0].allowed_values == (
        "none",
        "low",
        "medium",
        "high",
    ...
    assert fields["jaggedness"].schema[0].allowed_values == (
        "none",
        "low",
        "medium",
        "high",
...
def test_phrase_transform_params_reject_unknown_enum_value():
    ...
    phrase_config = {
        "motifs": ["seed"],
        "transforms": [
            {"name": "pad_silence", "params": {"seconds": 0.5, "position": "middle"}}
        ],
    }
    with pytest.raises(ValueError):
        render_phrase_from_config(phrase_config, parsed_motifs)
...
def test_parse_phrase_scale_applies_to_all_grouped_motifs():
    ...
    phrase_dict: PhraseConfigInput = {
        "motifs": ["seed_a", "seed_b"],
        "transforms": [
            {"name": "scale", "params": {"dimension": "duration", "factor": 2.0}}
        ],
    }
    result = render_phrase_from_config(phrase_dict, parsed_motifs)
    assert len(result) == 3
...
class TestScaleTransformParsing:
    ...
    def test_scale_with_missing_required_fields_raises_error(self):
        ...
        for required_field in (
            f for f, s in descriptor.params_spec.fields.items() if s.required
        ):
        ...
        for required_field in (
            ...
            composition_doc: CompositionDocumentInput = {
                ...
                "score": {
                    ...
                    "voices": [{"phrases": [{"motifs": ["seed"]}]}],
                    "score_transforms": [
                        {"name": "scale", "params": incomplete_params}
                    ],
                },
            }
            with pytest.raises(ValueError):
        ...
        for required_field in (
            ...
            composition_doc: CompositionDocumentInput = {
                ...
                "score": {
                    ...
                    "voices": [{"phrases": [{"motifs": ["seed"]}]}],
                    "score_transforms": [
                        {"name": "transpose", "params": incomplete_params}
                    ],
                },
            }
            with pytest.raises(ValueError):
    ...
    def test_required_param_transforms_reject_unknown_top_level_params(
        self, transform_name: str, valid_params: dict[str, float | int]
    ) -> None:
        parsed_motifs = {"seed": [Tone(440)]}
        phrase_config = {
            "motifs": ["seed"],
            "transforms": [
                {"name": transform_name, "params": {**valid_params, "unexpected": 123}}
            ],
        }
        with pytest.raises(ValueError):
            render_phrase_from_config(phrase_config, parsed_motifs)
        ...
        for required_field in (
            ...
            composition_doc: CompositionDocumentInput = {
                ...
                "score": {
                    ...
                    "voices": [{"phrases": [{"motifs": ["seed"]}]}],
                    "score_transforms": [
                        {"name": "delay", "params": incomplete_params}
                    ],
                },
            }
            with pytest.raises(ValueError):
        ...
        for required_field in (
            ...
            composition_doc: CompositionDocumentInput = {
                ...
                "score": {
                    ...
                    "voices": [{"phrases": [{"motifs": ["seed"]}]}],
                    "score_transforms": [
                        {"name": "repeat", "params": incomplete_params}
                    ],
                },
            }
            with pytest.raises(ValueError):
        ...
        for required_field in (
            ...
            composition_doc: CompositionDocumentInput = {
                ...
                "score": {
                    ...
                    "voices": [{"phrases": [{"motifs": ["seed"]}]}],
                    "score_transforms": [
                        {"name": "drift", "params": incomplete_params}
                    ],
                },
            }
            with pytest.raises(ValueError):
        ...
        for required_field in (
            ...
            composition_doc: CompositionDocumentInput = {
                ...
                "score": {
                    ...
                    "voices": [{"phrases": [{"motifs": ["seed"]}]}],
                    "score_transforms": [
                        {"name": "weierstrass", "params": incomplete_params}
                    ],
                },
            }
            with pytest.raises(ValueError):
    ...
    def test_cellular_automata_with_missing_required_fields_raises_error(self):
        ...
        for required_field in (
            ...
            phrase_config = {
                "motifs": ["seed"],
                "transforms": [
                    {"name": "cellular_automata", "params": incomplete_params}
                ],
            }
        ...
        for required_field in (
            ...
            composition_doc: CompositionDocumentInput = {
                ...
                "score": {
                    ...
                    "voices": [],
                    "score_transforms": [
                        {"name": "add_pedal_tone", "params": incomplete_params}
                    ],
                },
            }
            with pytest.raises(ValueError):
    ...
    def test_add_pedal_tone_applies_from_composition_json(self):
        ...
        composition_doc: CompositionDocumentInput = {
            ...
            "score": {
                ...
                "voices": [],
                "score_transforms": [
                    {"name": "add_pedal_tone", "params": {"frequency": pedal_tone}}
                ],
            },
        }
        score = transform_score(generate_score_plan(composition_doc))
        assert len(score.voices) == 1
        assert flatten_voice_tones(score.voices[0])[0].frequency == pytest.approx(
            pedal_tone
        )
    ...
    def test_golden_ratio_accepts_optional_dimension(self):
        ...
        phrase_config = {
            "motifs": ["seed"],
            "transforms": [
                {"name": "golden_ratio", "params": {"dimension": "frequency"}}
            ],
        }
        result = render_phrase_from_config(phrase_config, parsed_motifs)
            ...
            "voices": [
                {
                    "phrases": [
                        ...
                        {
                            "motifs": ["seed_c"],
                            "transforms": [{"name": "phrase_feigenbaum_shrink"}],
                        },
                    ]
                }
            ],
...
def test_generate_score_plan_phrase_relative_transforms_use_document_order():
    ...
    assert second_voice_tones[0].duration == pytest.approx(
        (2.0 + 2.0 * GOLDEN_RATIO) * GOLDEN_RATIO
    )
...
def test_generate_score_plan_score_transforms_must_be_list():
    invalid_document: object = {
        "name": "Composition Study",
        "score": {"motifs": {}, "voices": [], "score_transforms": {}},
    }
    with pytest.raises(ValueError):
        transform_score(generate_score_plan(invalid_document))
...
def test_generate_score_plan_score_transform_object_requires_name():
    json_data: object = {
        "name": "Composition Study",
        "score": {"motifs": {}, "voices": [], "score_transforms": [{"params": 2.0}]},
    }
    with pytest.raises(ValueError):
...
def test_generate_score_plan_score_reverse_applies_to_all_voices_without_params():
    ...
    score = transform_score(generate_score_plan(json_data))
    assert [tone.frequency for tone in flatten_voice_tones(score.voices[0])] == [
        660.0,
        440.0,
    ]
    assert [tone.duration for tone in flatten_voice_tones(score.voices[0])] == [
        0.25,
        0.5,
    ]
    assert [tone.frequency for tone in flatten_voice_tones(score.voices[1])] == [
        990.0,
        880.0,
    ]
    assert [tone.duration for tone in flatten_voice_tones(score.voices[1])] == [
        1.0,
        0.75,
    ]
...
def test_generate_score_plan_full_score_path():
    json_data: CompositionDocumentInput = {
        "name": "Composition Study",
        "score": {
            "motifs": {"seed_a": ["440:0.5"], "seed_b": ["880:0.5"]},
            "voices": [
                {
                    "phrases": [
                ...
                {"phrases": [{"motifs": ["seed_b"], "transforms": []}]},
            ],
...
def test_generate_score_plan_with_value_score_transform():
    ...
    json_data: CompositionDocumentInput = {
        "name": "Composition Study",
        "score": {
            "motifs": {"seed": [f"440:{original_duration}"]},
            "voices": [{"phrases": [{"motifs": ["seed"]}]}],
            "score_transforms": [
                {"name": "scale", "params": {"dimension": "duration", "factor": factor}}
            ],
    ...
    assert len(score.voices) == 1
    assert (
        flatten_voice_tones(score.voices[0])[0].duration == original_duration * factor
    )
...
def test_generate_score_plan_score_target_motifs_scope_receives_score_and_params_only():
    ...
    SCORE_TRANSFORMS["_test_score_with_motifs"] = ScoreTransformDefinition(
        ...
        params_spec=TransformParamsSpec(
            ...
            fields={
                "motif": TransformParamFieldSpec(schema=StringParam(), required=True)
            },
        ),
    ...
    try:
        valid_document: CompositionDocumentInput = {
            ...
            "score": {
                ...
                "voices": [],
                "score_transforms": [
                    {"name": "_test_score_with_motifs", "params": {"motif": "seed"}}
                ],
            },
        }
...
def test_generate_score_plan_score_target_motifs_scope_requires_params_object():
    ...
    SCORE_TRANSFORMS["_test_score_with_motifs"] = ScoreTransformDefinition(
        name="_test_score_with_motifs",
        transform_function=lambda score, params: noop_score_target_motifs_transform(
            score, params["motif"]
        ),
        params_spec=TransformParamsSpec(
            params_factory=lambda p: p.values,
...
def test_parse_dimension_rejects_unknown_dimension_with_valid_options_in_message():
    ...
    with pytest.raises(
        ValueError, match=f"Unknown dimension '{invalid_dimension}'. Valid options are:"
    ):
