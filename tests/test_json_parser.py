...
from transforms.base import EnumParam, ScoreTransformDefinition, StringParam, TransformParamFieldSpec, TransformParamsSpec
...
def render_phrase_from_config(phrase_config: object, parsed_motifs: dict[str, list[Tone]], reference_tones: list[Tone] | None=None) -> list[Tone]:
    motifs_section: MotifsConfigInput = {name: [f'{tone.frequency}:{tone.duration}' for tone in tones] for name, tones in parsed_motifs.items()}
...
@pytest.mark.parametrize(('transform_name', 'params'), [('delay', {'seconds': True}), ('delay', {'seconds': 'slow'}), ('repeat', {'count': True}), ('repeat', {'count': 1.5})])
def test_phrase_transform_params_reject_invalid_basic_types(transform_name, params):
    parsed_motifs = {'seed': [Tone(440)]}
    phrase_config = {'motifs': ['seed'], 'transforms': [{'name': transform_name, 'params': params}]}
    with pytest.raises(ValueError):
        render_phrase_from_config(phrase_config, parsed_motifs)

def test_phrase_transform_params_reject_unknown_enum_value():
    parsed_motifs = {'seed': [Tone(440)]}
    phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'pad_silence', 'params': {'seconds': 0.5, 'position': 'middle'}}]}
    with pytest.raises(ValueError):
        render_phrase_from_config(phrase_config, parsed_motifs)

@pytest.mark.parametrize('strength', ['high', 0.75])
def test_union_transform_params_accept_enum_or_float_values(strength):
    parsed_motifs = {'seed': [Tone(440, 0.5), Tone(660, 0.5)]}
    phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'accelerando', 'params': {'strength': strength, 'jaggedness': 'none'}}]}
    result = render_phrase_from_config(phrase_config, parsed_motifs)
    assert len(result) == 2

@pytest.mark.parametrize('strength', ['wild', True, []])
...
def test_parse_motifs():
    motifs_dict = {'seed_a': ['440:0.25', '880'], 'seed_b': ['523.25']}
...
def test_parse_phrase_single_motif_from_motifs_list():
    parsed_motifs = {'seed_a': [Tone(440, 0.5), Tone(880, 0.5)]}
    phrase_dict: PhraseConfigInput = {'motifs': ['seed_a'], 'transforms': [{'name': 'reverse'}]}
...
def test_parse_phrase_multiple_motifs():
    parsed_motifs = {'seed_a': [Tone(440, 0.5)], 'seed_b': [Tone(880, 0.25), Tone(523.25, 0.75)]}
    phrase_dict: PhraseConfigInput = {'motifs': ['seed_a', 'seed_b']}
    result = render_phrase_from_config(phrase_dict, parsed_motifs)
    assert len(result) == 3
...
def test_parse_phrase_reverse_applies_after_grouping_motifs():
    parsed_motifs = {'seed_a': [Tone(440, 0.5), Tone(660, 0.25)], 'seed_b': [Tone(880, 0.75)]}
    phrase_dict: PhraseConfigInput = {'motifs': ['seed_a', 'seed_b'], 'transforms': [{'name': 'reverse'}]}
    result = render_phrase_from_config(phrase_dict, parsed_motifs)
    assert len(result) == 3
...
def test_parse_phrase_scale_applies_to_all_grouped_motifs():
    parsed_motifs = {'seed_a': [Tone(440, 0.5)], 'seed_b': [Tone(880, 0.25), Tone(523.25, 0.75)]}
    phrase_dict: PhraseConfigInput = {'motifs': ['seed_a', 'seed_b'], 'transforms': [{'name': 'scale', 'params': {'dimension': 'duration', 'factor': 2.0}}]}
    result = render_phrase_from_config(phrase_dict, parsed_motifs)
    assert len(result) == 3
...
def test_parse_phrase_delay_applies_to_all_grouped_motifs():
    ...
    phrase_dict: PhraseConfigInput = {'motifs': ['seed_a'], 'transforms': [{'name': 'delay', 'params': {'seconds': DELAY_SECONDS}}]}
    result = render_phrase_from_config(phrase_dict, parsed_motifs)
...
class TestScaleTransformParsing:
    ...
    def test_scale_duration(self):
        ...
        factor = 0.5
        phrase_dict: PhraseConfigInput = {'motifs': ['seed_a'], 'transforms': [{'name': 'scale', 'params': {'dimension': 'duration', 'factor': factor}}]}
        result = render_phrase_from_config(phrase_dict, parsed_motifs)
        assert len(result) == 1
    ...
    def test_scale_frequency(self):
        ...
        phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'scale', 'params': {'dimension': 'frequency', 'factor': factor}}]}
        result = render_phrase_from_config(phrase_config, parsed_motifs)
        assert len(result) == 1
    ...
    def test_scale_amplitude(self):
        ...
        phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'scale', 'params': {'dimension': 'amplitude', 'factor': factor}}]}
        result = render_phrase_from_config(phrase_config, parsed_motifs)
        assert len(result) == 1
    ...
    def test_scale_with_numeric_param_raises_error(self):
        parsed_motifs = {'seed': [Tone(440)]}
        phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'scale', 'params': 2.0}]}
        with pytest.raises(ValueError):
            render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_scale_with_missing_required_fields_raises_error(self):
        ...
        for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
            ...
            phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'scale', 'params': incomplete_params}]}
            with pytest.raises(ValueError):
                render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_transpose_with_missing_required_fields_raises_error(self):
        ...
        for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
            incomplete_params = valid_params.copy()
            incomplete_params.pop(required_field)
            phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'transpose', 'params': incomplete_params}]}
            with pytest.raises(ValueError):
                render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_reverse_rejects_unknown_top_level_params(self):
        parsed_motifs = {'seed': [Tone(440)]}
        phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'reverse', 'params': {'unexpected': True}}]}
        with pytest.raises(ValueError):
            render_phrase_from_config(phrase_config, parsed_motifs)

    @pytest.mark.parametrize(('transform_name', 'valid_params'), [('transpose', {'semitones': 1.0}), ('delay', {'seconds': 0.25}), ('repeat', {'count': 2})])
    def test_required_param_transforms_reject_unknown_top_level_params(self, transform_name: str, valid_params: dict[str, float | int]) -> None:
        parsed_motifs = {'seed': [Tone(440)]}
        phrase_config = {'motifs': ['seed'], 'transforms': [{'name': transform_name, 'params': {**valid_params, 'unexpected': 123}}]}
        with pytest.raises(ValueError):
            render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_delay_with_missing_required_fields_raises_error(self):
        ...
        for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
            incomplete_params = valid_params.copy()
            incomplete_params.pop(required_field)
            phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'delay', 'params': incomplete_params}]}
            with pytest.raises(ValueError):
                render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_repeat_with_missing_required_fields_raises_error(self):
        ...
        for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
            incomplete_params = valid_params.copy()
            incomplete_params.pop(required_field)
            phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'repeat', 'params': incomplete_params}]}
            with pytest.raises(ValueError):
                render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_accelerando_with_missing_required_fields_raises_error(self):
        ...
        for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
            incomplete_params = valid_params.copy()
            incomplete_params.pop(required_field)
            phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'accelerando', 'params': incomplete_params}]}
            with pytest.raises(ValueError):
                render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_ritardando_with_missing_required_fields_raises_error(self):
        ...
        for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
            incomplete_params = valid_params.copy()
            incomplete_params.pop(required_field)
            phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'ritardando', 'params': incomplete_params}]}
            with pytest.raises(ValueError):
                render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_drift_with_missing_required_fields_raises_error(self):
        ...
        for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
            incomplete_params = valid_params.copy()
            incomplete_params.pop(required_field)
            phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'drift', 'params': incomplete_params}]}
            with pytest.raises(ValueError):
                render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_weierstrass_with_missing_required_fields_raises_error(self):
        ...
        valid_params = {'dimension': 'frequency', 'intensity': 'medium'}
        for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
            incomplete_params = valid_params.copy()
            incomplete_params.pop(required_field)
            phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'weierstrass', 'params': incomplete_params}]}
            with pytest.raises(ValueError):
                render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_cellular_automata_with_missing_required_fields_raises_error(self):
        ...
        valid_params = {'dimension': 'frequency', 'rule': 30, 'generations': 5, 'max_deviation': 0.3}
        for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
            incomplete_params = valid_params.copy()
            incomplete_params.pop(required_field)
            phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'cellular_automata', 'params': incomplete_params}]}
    ...
    def test_erosion_accepts_optional_dimension(self):
        ...
        phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'erosion', 'params': {'dimension': 'amplitude'}}]}
        result = render_phrase_from_config(phrase_config, parsed_motifs)
        assert result[1].amplitude < 1.0

    def test_golden_ratio_accepts_optional_dimension(self):
        parsed_motifs = {'seed': [Tone(440, 1.0)]}
        phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'golden_ratio', 'params': {'dimension': 'frequency'}}]}
        result = render_phrase_from_config(phrase_config, parsed_motifs)
    ...
    def test_erosion_rejects_unknown_param(self):
        parsed_motifs = {'seed': [Tone(440)]}
        phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'erosion', 'params': {'unexpected': 123}}]}
        with pytest.raises(ValueError):
            render_phrase_from_config(phrase_config, parsed_motifs)

def test_parse_phrase_with_reference_transform():
    parsed_motifs = {'seed_a': [Tone(440, 0.5)]}
    reference_tones = [Tone(110, 1.0)]
    phrase_dict: PhraseConfigInput = {'motifs': ['seed_a'], 'transforms': [{'name': 'phrase_relative_golden_ratio_grow'}]}
...
def test_parse_phrase_reference_transform_uses_total_grouped_phrase_duration():
    parsed_motifs = {'seed_a': [Tone(440, 0.5)], 'seed_b': [Tone(880, 0.5)]}
    reference_tones = [Tone(110, 2.0)]
    phrase_dict: PhraseConfigInput = {'motifs': ['seed_a', 'seed_b'], 'transforms': [{'name': 'phrase_relative_golden_ratio_grow'}]}
    result = render_phrase_from_config(phrase_dict, parsed_motifs, reference_tones)
    assert len(result) == 2
...
def test_parse_phrase_rejects_score_transform_in_phrase_transforms():
    ...
    phrase_config = {'motifs': ['seed'], 'transforms': [{'name': 'add_pedal_tone'}]}
    with pytest.raises(ValueError):
        render_phrase_from_config(phrase_config, parsed_motifs)
...
def test_stretto_with_missing_required_fields_raises_error():
    ...
    valid_params = {'motif': 'subject', 'num_times': 2, 'spacing': 'golden_ratio'}
    for required_field in (f for f, s in descriptor.params_spec.fields.items() if s.required):
        incomplete_params = valid_params.copy()
        incomplete_params.pop(required_field)
