...
- **`golden_ratio` / `feigenbaum_sequence`**: Applies mathematical constants to a phrase or group of phrases so that they are related proportionally using these ratios. Both optionally accept `dimension`.
  - **`phrase_relative_golden_ratio_grow`**: Grows the phrase it is applied to by a proportion of the Golden Ratio to the previous phrase in the same voice.
  - **`phrase_relative_golden_ratio_shrink`**: Shrinks the phrase it is applied to by a proportion of the Golden Ratio to the previous phrase in the same voice.
