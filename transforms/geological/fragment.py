...
class FragmentParams:
    damage_pct: int
    damage_tones_span: int # how wide a span of tones to damage throughout the phrase
...
def _create_fragment_params(parsed_params: ParsedTransformParams) -> FragmentParams:
    ...
    damage_tones_span = parsed_params.required("damage_tones_span", int)
    damage_pattern_key = parsed_params.required("damage_pattern_key", (str, type(None)))

    _validate_fragment_params(damage_pct=damage_pct, damage_tones_span=damage_tones_span)

    return FragmentParams(
        damage_pct=damage_pct,
        damage_tones_span=damage_tones_span,
        damage_pattern_key=damage_pattern_key,
    )


def _validate_fragment_params(damage_pct: int, damage_tones_span: int) -> None:
    if damage_pct < 0 or damage_pct > 100:
        raise ValueError(f"damage_pct must be between 0 and 100, got {damage_pct}")
    if damage_tones_span < 1:
        raise ValueError(f"damage_tones_span must be at least 1, got {damage_tones_span}")
...
FRAGMENT_PARAMS_SPEC = TransformParamsSpec[FragmentParams](
    ...
    fields={
        ...
        "damage_tones_span": TransformParamFieldSpec(
            required=True,
            schema=IntegerParam(),
        ),
...
def fragment_phrase_transform(context: PhraseTransformContext, params: FragmentParams) -> Phrase:
    ...
    transformed_tones = fragment_transform(
        ...
        damage_tones_span=params.damage_tones_span,
